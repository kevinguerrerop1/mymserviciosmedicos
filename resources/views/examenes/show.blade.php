...
@section('content')
    ...
    <div class="row g-4">
                    ...
                    <table class="table table-bordered mb-0 align-middle">
                        <tbody>
                            <tr>
                                ...
                                <td class="fw-bold text-dark">{{ $examen->paciente_nombre }} <code
                                        class="ms-1">({{ $examen->paciente_rut }})</code></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="fw-bold">{{ \Carbon\Carbon::parse($examen->fecha_recepcion)->format('d/m/Y') }}
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($examen->numero_fragmentos)
                                        <span class="badge bg-light text-dark border ms-1">{{ $examen->numero_fragmentos }}
                                            Fragmento(s)</span>
                                    @endif
                                </td>
                            </tr>
                    ...
                    @if ($examen->galeria_imagenes && count($examen->galeria_imagenes) > 0)
                        @foreach ($examen->galeria_imagenes as $img)
                            <a href="{{ asset('storage/' . $img) }}" target="_blank"
                                class="shadow-sm rounded overflow-hidden border">
                    ...
                    <div class="card border-success shadow-sm">
                        ...
                        <div class="card-body bg-white text-center py-3">
                            <p class="mb-2 text-muted small">
                                Este examen ya ha sido marcado como <strong>{{ $examen->estado }}</strong>. El expediente se
                                encuentra bloqueado para modificaciones médicas.
                            </p>

                            @hasrole('admin')
                                <button type="button" class="btn btn-outline-danger btn-sm mt-1 fw-bold" data-bs-toggle="modal"
                                    data-bs-target="#modalReabrirExamen">
                            ...
                            <form action="{{ route('examenes.update', $examen->id) }}" method="POST"
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="estado" id="selectEstado" class="form-select">
                                        <option value="PENDIENTE" {{ $examen->estado == 'PENDIENTE' ? 'selected' : '' }}>
                                            PENDIENTE</option>
                                        <option value="EN ESPERA INFORME COMPLEMENTARIO"
                                            {{ $examen->estado == 'EN ESPERA INFORME COMPLEMENTARIO' ? 'selected' : '' }}>EN
                                            ESPERA INFORME COMPLEMENTARIO</option>

                                        <option value="INFORMADO RESULTADO CRÍTICO"
                                            {{ $examen->estado == 'INFORMADO RESULTADO CRÍTICO' ? 'selected' : '' }}
                                            {{ !$tieneInforme ? 'disabled' : '' }} class="opcion-requiere-pdf">
                                            INFORMADO RESULTADO CRÍTICO {{ !$tieneInforme ? '(Requiere PDF)' : '' }}
                                        </option>

                                        <option value="INFORMADO" {{ $examen->estado == 'INFORMADO' ? 'selected' : '' }}
                                            {{ !$tieneInforme ? 'disabled' : '' }} class="opcion-requiere-pdf">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="file" name="archivo_informe" id="inputPdf" class="form-control"
                                        accept=".pdf">
                                </div>
                                ...
                                <div class="col-12 text-end">
                                    <button type="submit" class="btn btn-warning fw-bold px-4">Guardar Cambios y
                                        Notificar</button>
                                </div>
                            </form>
                        </div>
        ...
        <div class="col-12 text-center">
            <div class="p-3 bg-white border rounded shadow-sm">
                @if ($examen->archivo_informe)
                    <a href="{{ route('examenes.descargar', $examen->id) }}"
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 trazabilidad-card overflow-hidden">
                ...
                <div class="card-header bg-primary text-white py-3 px-4 d-flex justify-content-between align-items-center">
                    ...
                    <button type="button"
                        class="btn btn-sm btn-light fw-bold text-primary shadow-sm rounded-pill px-3 py-1.5 d-flex align-items-center gap-1.5"
                        data-bs-toggle="modal" data-bs-target="#modalNotasExamen">
                        <span>💬 Ver Comentarios / Notas</span>
                        @if ($notasManuales->count() > 0)
                            <span class="badge bg-danger text-white rounded-pill ms-1"
                                style="font-size: 0.75rem;">{{ $notasManuales->count() }}</span>
                        @endif
                    </button>
                </div>
                ...
                <div class="card-body p-4">
                        ...
                        <div class="timeline-track">
                            @foreach ($eventosSistema as $hito)
                                <div class="timeline-step">
                                    <!-- NODO DE HISTORIAL -->
                                    <div
                                        class="timeline-icon {{ $loop->first ? 'timeline-icon-latest' : 'timeline-icon-history' }}">
                                        @if ($loop->first)
                                            ✓
                                        @else
                                            •
                                    ...
                                    <div class="timeline-content-card p-3">
                                        <div
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <span
                                                    class="badge {{ $loop->first ? 'bg-primary text-white' : 'bg-secondary-subtle text-dark border' }} rounded-pill px-2.5 py-1 font-monospace fw-semibold"
                                            ...
                                            <div class="text-muted d-flex align-items-center gap-1"
                                                ...
                                                <span
                                                    class="fw-bold text-dark">{{ $hito->created_at->format('d/m/Y') }}</span>
                                                <span>a las</span>
                                                <span class="fw-bold text-dark">{{ $hito->created_at->format('H:i') }}
                                                    hrs</span>
                                                <span
                                                    class="text-muted ps-1">({{ $hito->created_at->diffForHumans() }})</span>
                                            </div>
                                        </div>
                                        <!-- DESCRIPCIÓN DEL EVENTO -->
                                        <p class="mb-0 text-dark font-semibold text-break mt-1"
                                            style="line-height: 1.45; font-size: 0.88rem;">
                                            {{ $hito->comentario }}
                                        </p>
                                    </div>
                                </div>
                    ...
                    @else
                        <div class="text-center text-muted py-4">
                            <p class="mb-0 small fw-semibold">No hay registros de cambios de estado ni trazabilidad aún.
                            </p>
                        </div>
                    @endif
                </div>
        ...
        <div class="modal fade" id="modalNotasExamen" tabindex="-1" aria-labelledby="modalNotasExamenLabel"
            aria-hidden="true">
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body bg-white p-4">
                        <div class="mb-4">
                            ...
                            @empty
                                <div class="text-center text-muted py-4 bg-light rounded border border-dashed">
                                    <p class="mb-0 small">No hay comentarios u observaciones registradas para este examen.
                                    </p>
                                </div>
                            @endforelse
                        </div>
                        ...
                        <form action="{{ route('comentarios.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label for="comentario" class="form-label fw-bold small text-muted">NUEVA
                                    OBSERVACIÓN</label>
                                <textarea name="comentario" id="comentario" class="form-control" rows="3"
                                    placeholder="Escriba aquí cualquier aclaración sobre la muestra o notas del equipo médico..." required></textarea>
                            </div>

                            <div class="text-end">
                                <button type="submit" class="btn btn-primary btn-sm px-4 fw-bold">Guardar
                                    Comentario</button>
                            </div>
                        </form>
                    </div>
                        ...
                        <form action="{{ route('examenes.reabrir', $examen->id) }}" method="POST">
                            ...
                            <div class="modal-body bg-white">
                                <p class="small text-muted">
                                    Esta acción devolverá el examen a estado <strong>EN ESPERA INFORME COMPLEMENTARIO</strong>
                                    para permitir rectificaciones, reemplazo de PDF o carga de imágenes.
                                </p>
                                <div class="mb-3">
                                    <label for="motivo" class="form-label fw-bold small text-dark">MOTIVO O JUSTIFICACIÓN
                                        (Obligatorio para auditoría)</label>
                                    <textarea name="motivo" id="motivo" class="form-control" rows="3"
                                        placeholder="Ej: Corregir informe adjunto / Reemplazar PDF por error tipográfico..." required minlength="10"></textarea>
                                </div>
                            </div>
